...
#Time series forecasing for mean discharge rate (cft/s) of San Marcos on 
...
#Loading csv file
"""
file_path=tf.keras.utils.get_file(origin='https://drive.google.com/uc?export=download&id=1oB6mjx07xlDUoviNkWRFu9GI0oI97Bnw')
csv_path, _ =os.path.splitext(file_path)
df=pd.read_csv(csv_path)
"""
os.chdir("E:/Machine Learning/AssignTS_forecast") #Change working directory
df=pd.read_csv("SanMarcos.csv")
...
plt.title("Mean discharge rate of San Marcos")
plt.xlabel("Year")
...
#Defining layers
model_SM=Sequential()
model_SM.add(InputLayer((6,1)))
model_SM.add(LSTM(64,activation='relu'))
model_SM.add(Dense(8,'relu'))
model_SM.add(Dense(1))

print("Model summary:", model_SM.summary())

#Defining checkpoint to save the state of the system
cp=ModelCheckpoint('model_SM/', save_best_only=True) 

#Configuring the model for training
model_SM.compile(loss=MeanSquaredError(), optimizer=Adam(learning_rate=0.001), metrics=[RootMeanSquaredError()])

#Train and fit the model
model_SM.fit(X_train, y_train, validation_data=(X_val, y_val), epochs=30, callbacks=[cp])


#Loading model
model_SM=load_model('model_CS/')

#Predicting test series (prediction vs actual)
test_predictions=model_SM.predict(X_test).flatten()
...
plt.title("Mean discharge rate of San Marcos (predicted and actual)")
